...
using Il2CppMethodInfo = LombexMenu.Protection.EventLogic.Il2CppMethodInfo;

namespace Protection.EventLogic.Sanitizers
{
    internal class FlatBufferSanitizer : ISanitizer
    {
        ...
        public FlatBufferSanitizer()
        {
            ...
            {
                if (!string.IsNullOrEmpty(decodeMethodName)) break;
                if (methodInfo.IsAbstract) continue;
                if (!methodInfo.Name.StartsWith("Method_Public_Virtual_Final_New_Void_ValueTypePublicSealed")) continue;
                foreach (var xi in XrefScanner.XrefScan(methodInfo))
                {
                    if (xi.Type != XrefType.Method) continue;
            ...
            {
                SetConsoleColor.WriteEmbeddedColorLine("Unable to determine target method name, you'll be unprotected against photon exploits.", SetConsoleColor.ConsoleLogType.Error);
                return;
            }
            unsafe
            {
            ...
            LombexMenu.Main.Harmony.Patch(typeof(FlatBufferNetworkSerializer).GetMethod(nameof(FlatBufferNetworkSerializer.Method_Public_Void_EventData_0)), typeof(FlatBufferSanitizer).GetMethod(nameof(FlatBufferNetworkSerializeReceivePatch), BindingFlags.NonPublic | BindingFlags.Static).ToNewHarmonyMethod());
        }
        private static bool FlatBufferNetworkSerializeReceivePatch(EventData __0)
        {
            if (__0.Code == 9 && RateLimiter.IsRateLimited(__0.Sender)) return false;
            return true;
        }
        private static void SyncPhysicsDispatchedUpdatePatch(IntPtr thisPtr, float param1, float param2, IntPtr nativeMethodInfo) => SafeDispatchedUpdate(_ourSyncPhysicsDispatchedUpdateMethodInfo, _ourSyncPhysicsDispatchedUpdate, thisPtr, param1, param2);
        private static void PoseRecorderDispatchedUpdatePatch(IntPtr thisPtr, float param1, float param2, IntPtr nativeMethodInfo) => SafeDispatchedUpdate(_ourPoseRecorderDispatchedUpdateMethodInfo, _ourPoseRecorderDispatchedUpdate, thisPtr, param1, param2);
        private static unsafe void SafeDispatchedUpdate(IntPtr ourMethodInfoPtr, IntPtr ourMethodPtr, IntPtr thisPtr, float param1, float param2)
        {
        ...
        private static void SyncPhysicsDecodePatch(IntPtr thisPtr, IntPtr objectsPtr, int objectIndex, float sendTime, IntPtr nativeMethodInfo) => SafeDecode(_ourSyncPhysicsDecodeMethodInfo, _ourSyncPhysicsDecode, thisPtr, objectsPtr, objectIndex, sendTime);
        private static void AvatarPlayableControllerDecodePatch(IntPtr thisPtr, IntPtr objectsPtr, int objectIndex, float sendTime, IntPtr nativeMethodInfo) => SafeDecode(_ourAvatarPlayableDecodeMethodInfo, _ourAvatarPlayableDecode, thisPtr, objectsPtr, objectIndex, sendTime);
        private static void PoseRecorderDecodePatch(IntPtr thisPtr, IntPtr objectsPtr, int objectIndex, float sendTime, IntPtr nativeMethodInfo) => SafeDecode(_ourPoseRecorderDecodeMethodInfo, _ourPoseRecorderDecode, thisPtr, objectsPtr, objectIndex, sendTime);
        ...
        {
            if (SafeInvokeDecode(ourMethodInfoPtr, ourMethodPtr, thisPtr, objectsPtr, objectIndex, sendTime)) return;
        ...
        {
            if (RateLimiter.IsRateLimited(eventData.Sender)) return true;
