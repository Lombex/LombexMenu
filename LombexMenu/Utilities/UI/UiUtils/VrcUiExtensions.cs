...
namespace ReMod.Core.VRChat
{
    public static class VrcUiExtensions
    {
        ...
        public static void QueueHudMessage(this VRCUiManager UiManager, string notification, string color = "#FFFFFF", float Height = 350f, float duration = 5f, float delay = 0f)
        {
            UiManager.field_Public_Text_0.alignment = TextAnchor.UpperCenter;
            UiManager.field_Public_Text_0.rectTransform.anchoredPosition = new Vector2(0f, Height);
            UiManager.field_Public_Text_0.color = Utils.Utilities.ToColor(color);
            ...
            bool clearPageStack = false, UIPage.TransitionType transitionType = UIPage.TransitionType.Right)
        {
            if (_pushPage == null)
            {
                _pushPage = (PushPageDelegate)Delegate.CreateDelegate(typeof(PushPageDelegate),
                    typeof(MenuStateController).GetMethods().FirstOrDefault(m => m.GetParameters().Length == 4 && m.Name.StartsWith("Method_Public_Void_String_UIContext_Boolean_TransitionType_") && Utils.XRefScanners.CheckMethod(m, "No page named")));
            }
            ...
            bool clearPageStack = false, bool inPlace = false)
        {
            if (_switchToRootPage == null)
            {
                _switchToRootPage = (SwitchToRootPageDelegate)Delegate.CreateDelegate(typeof(SwitchToRootPageDelegate),
                    typeof(MenuStateController).GetMethods().FirstOrDefault(m => m.Name.StartsWith("Method_Public_Void_String_UIContext_Boolean_") && Utils.XRefScanners.CheckMethod(m, "UIPage not in root page list:")));
            }
        ...
        public static void CloseUi(this VRCUiManager uiManager)
        {
            if (_closeUi == null)
            {
                _closeUi = (CloseUiDelegate)Delegate.CreateDelegate(typeof(CloseUiDelegate),
                    typeof(VRCUiManager).GetMethods().FirstOrDefault(m => m.Name.StartsWith("Method_Public_Void_Boolean_Boolean") && !m.Name.Contains("PDM") && Utils.XRefScanners.CheckUsing(m, "TrimCache")));
            }
        ...
        public static void ShowUi(this VRCUiManager uiManager, bool showDefaultScreen = true, bool showBackdrop = true)
        {
            if (_showUi == null)
            {
                _showUi = (ShowUiDelegate)Delegate.CreateDelegate(typeof(ShowUiDelegate), typeof(VRCUiManager).GetMethods()
                    .First(mb => mb.Name.StartsWith("Method_Public_Void_Boolean_Boolean_") && !mb.Name.Contains("_PDM_") && Utils.XRefScanners.CheckMethod(mb, "UserInterface/MenuContent/Backdrop/Backdrop")));
            }
        ...
        {
            if (_radioSetTitleMethods == null) typeof(RadioButtonSelector).GetMethods(BindingFlags.Public | BindingFlags.Instance)
